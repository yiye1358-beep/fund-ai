...
<style>
    ...
    .stRadio label, .stSelectbox label { font-size: 1.1rem; }
...
st.title("📊 基金控制台 Pro")
st.caption("多源实时快讯 · 持仓盈亏分析 · 资金流向 · AI 解读")
...
@st.cache_data(ttl=90)
def fetch_tonghuashun_news():
    """同花顺财经快讯"""
    try:
        df = ak.stock_info_global_ths()
        if df is not None and not df.empty:
            return [str(row.get("content", "")) for _, row in df.iterrows() if row.get("content")][:15]
    except:
        return []

def fetch_cls_telegraph_legacy():
    """财联社电报（原接口）"""
    ...
    try:
        ...
        return [it.get_text(strip=True) for it in items[:10] if it.get_text(strip=True)]
    except:
        return []
...
def fetch_all_news():
    ...
    all_news = []
    seen = set()
    for news_list in [fetch_eastmoney_news(), fetch_tonghuashun_news(), fetch_cls_telegraph_legacy()]:
        for n in news_list:
...
def compute_holdings(holdings):
    ...
    for h in holdings:
        ...
        if nav_data and nav_data.get("单位净值") is not None:
            ...
            results.append({
                "代码": h['代码'], "名称": h['名称'], "成本价": h['成本'],
                "当前净值": "获取失败", "份额": h['份额'], "市值": None,
...
with st.sidebar:
    ...
    if st.session_state.show_news:
        if st.button("刷新多源快讯", use_container_width=True):
            with st.spinner("聚合多家快讯中..."):
                st.session_state.news_cache = fetch_all_news()
        st.caption("数据源：东方财富、同花顺、财联社")
...
if st.session_state.show_news:
    ...
    if st.session_state.news_cache:
        selected_news = st.selectbox("选择一条快讯进行AI分析", st.session_state.news_cache)
        if selected_news:
...
st.markdown("---")
st.caption("⚠️ 数据来源：东方财富、同花顺、财联社公开接口，仅供个人学习研究，不构成投资建议。")
